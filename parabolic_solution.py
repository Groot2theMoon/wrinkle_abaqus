...
def visualize_grid(points_df, output_filename="ground_truth_plot_1d.png"):
    ...
    if not success_points.empty:
        success_points = success_points.sort_values(by='depth')
        ax.plot(success_points['depth'], success_points['objective'],
                marker='o', markersize=5, linestyle='-', color='blue',label='Success')

    if not failure_points.empty:
        y_val_failure = ax.get_ylim()[0]
        ax.scatter(failure_points['depth'], [y_val_failure] * len(failure_points),
