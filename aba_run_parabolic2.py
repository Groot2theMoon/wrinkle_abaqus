...
ANALYSIS_PARAMETERS = {
    ...
    "APPLIED_DISPLACEMENT_RIKS": -0.005,
    "MESH_SIZE_PRE_BUCKLE": 0.001,
    "MESH_SIZE_POST_BUCKLE": 0.0025,
    "INITIAL_INCREMENT_SIZE": 0.1,
...
def cleanup_abaqus_files(base_job_name):
    ...
    for f_path in files_to_delete:
        if not f_path.endswith(('.py', '.csv', '.txt')):
            try:
                os.remove(f_path)
            except OSError as e:
...
def run_linear_buckle_analysis(job_name, depth):
    ...
    bc = model.DisplacementBC(name='LoadBC', createStepName='Preload', region=instance.sets['Disp_Edges'], u1=-1e-9, u2=0, u3=0, ur1=0, ur2=0, ur3=0)
...
def run_post_buckle_analysis(job_name, depth, buckle_job_name):
    ...
    instance = model.rootAssembly.instances['SheetInst']
    model.StaticRiksStep(name='Riks', previous='Initial', nlgeom=ON, maxNumInc=ANALYSIS_PARAMETERS["RIKS_MAX_INCREMENTS"], initialArcInc=ANALYSIS_PARAMETERS["RIKS_INITIAL_ARC_INC"])
    model.EncastreBC(name='FixLeft', createStepName='Initial', region=instance.sets['Fix_Edges'])
    ...
    with open(inp_path, 'w') as f_out:
        for line in lines:
            if line.strip().lower().startswith('*step'):
                imperfection_scale = ANALYSIS_PARAMETERS["SHEET_THICKNESS"] * 0.01
